...
- **Testable seam:** Parameterless `PatchReactions()` (called by `Plugin.Awake`) passes the real `Combustion.Result*` statics into `PatchReactions(methaneOxygen, methaneOzone)`, which applies the corrections via the shared `Patch(instance, fuel, oxidiser, outputs)` helper. Tests call the two-arg overload with **freshly constructed** instances, so they never mutate the shared game statics. Each test also asserts the fresh instance `ShouldBeEquivalentTo` the corresponding real static (read-only) to pin the assumed source recipe.
