...
4. **`StartingFuelMixturePatch.cs`** — Patches `SpawnData.Initialize` and permanently corrects matching recipes in each deserialized spawn-data tree before the game initializes it.
5. **`StartingFuelMixture.cs`** — Exact matching, cloning, and recursive traversal for 2:1 methane/oxygen spawn mixtures. It swaps the quantities to 1:2 while preserving total quantity.
...
- **Starting fuel patch modifies loaded spawn data:** The patch requires exactly one oxygen and one volatiles `GasAction`, mole units, matching explicit temperatures, no energy override or other gases, and the exact vanilla 2:1 ratio. It permanently replaces matching recipes during `SpawnData.Initialize`, before action initialization, so tooltips and spawned contents both use the corrected data. This covers vanilla and custom spawn definitions and defaults off.
